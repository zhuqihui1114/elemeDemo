<template>
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span @click="select(1,$event)" class="block negative" :class="{'active':myselectType===1}">{{desc.negative}}<span
  ...
  export default {
    ...
    methods: {
      select(type, event) {
        ...
        this.$root.event.$emit('ratingtype.select', type);
      },
      toggleContent(event) {
        this.myonlyContent = !this.myonlyContent;
        this.$root.event.$emit('content.toggle', this.myonlyContent);
      }
    }
  };
</script>
